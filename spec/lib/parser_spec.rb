RSpec.describe Annotator::Parser do
  ...
  #   describe option do
  #     it 'sets the ENV variable' do
  #       expect { described_class.parse([option]) }.to output("annotate v#{Annotate.version}\n").to_stdout
  #       expect(described_class.parse([option])).to include(exit: true)
  #     end
  #   end
  # end
  ...
  describe 'described_class::EXCLUSION_LIST' do
    it "has 'tests'" do
      expect(described_class::EXCLUSION_LIST).to include('tests')
    end

    it "has 'fixtures'" do
      expect(described_class::EXCLUSION_LIST).to include('fixtures')
    end

    it "has 'factories'" do
      expect(described_class::EXCLUSION_LIST).to include('factories')
    end

    it "has 'serializers'" do
      expect(described_class::EXCLUSION_LIST).to include('serializers')
    end
  end
  ...
  %w[-f --format].each do |option|
    describe option do
      described_class::FORMAT_TYPES.each do |format_type|
